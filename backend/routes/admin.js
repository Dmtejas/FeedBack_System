...
// Admin login with category
router.post('/login', async (req, res) => {
  const { username, password, category } = req.body; // <-- get category from login request
  const admin = await Admin.findOne({ username });
  if (!admin || !(await admin.comparePassword(password))) {
    res.status(401)
  ...
  // Save category in token (can override if needed)
  const token = jwt.sign(
    { id: admin._id, category: category || admin.category }, 
    process.env.SECRET_KEY,
    { expiresIn: '1d' }
  );

  res.json({ token, category: category || admin.category });
});
...
router.get('/dashboard', auth, async (req, res) => {
  ...
  switch (category) {
    case 'Teacher Feedback':
      Model = require('../models/teacherFeedback');
      break;
    case 'Product Review':
      Model = require('../models/productReview');
      break;
    case 'Service Evaluation':
      Model = require('../models/serviceEvaluation');
      break;
    case 'Event Feedback':
      Model = require('../models/eventFeedback');
      break;
    default:
  ...
  try {
    const feedbacks = await Model.find().sort({ createdAt: -1 }).limit(20);
    ...
    if (category === "Teacher Feedback") {
      ...
      questions.forEach(q => {
        ...
        feedbacks.forEach(fb => {
          if (fb[q]) {
            dist[q][fb[q]]++;
            const map = { excellent: 4, good: 3, average: 2, poor: 1 };
            sum += map[fb[q]] || 0;
            n++;
          }
        });
      ...
      feedbacks.forEach(fb => {
        if (fb.rating) {
          dist.rating[fb.rating]++;
          const map = { excellent: 4, good: 3, average: 2, poor: 1 };
          sum += map[fb.rating] || 0;
          n++;
        }
        if (fb.recommend) dist.recommend[fb.recommend]++;
      });
      avg.rating = n ? (sum / n) : 0;
    } else if (category === "Service Evaluation") {
      dist.satisfaction = { 'very-satisfied': 0, satisfied: 0, neutral: 0, dissatisfied: 0, 'very-dissatisfied': 0 };
    ...
    feedbacks.forEach(fb => {
      const sentimentField = {
        "Teacher Feedback": 'suggestionSentiment',
        "Product Review": 'commentSentiment',
        "Service Evaluation": 'improvementSentiment',
        "Event Feedback": 'commentSentiment'
      }[category];

      if (typeof fb[sentimentField] === 'number') {
        sentimentSum += fb[sentimentField];
        sentimentCount++;
      }
    });
    const avgSentiment = sentimentCount ? (sentimentSum / sentimentCount) : 0;

    // Last 10 feedbacks for trend
    const trendFeedbacks = await Model.find().sort({ createdAt: -1 }).limit(10);
    const sentimentTrend = trendFeedbacks
      .map(fb => fb.suggestionSentiment ?? fb.commentSentiment ?? fb.improvementSentiment ?? 0)
      .reverse();
    const sentimentTrendLabels = trendFeedbacks
      .map(fb => new Date(fb.createdAt).toLocaleDateString('en-GB', { month: 'short', day: 'numeric' }))
      .reverse();

    res.json({ count, avg, dist, avgSentiment, feedbacks, sentimentTrend, sentimentTrendLabels });
  } catch (err) {
    res.status(500).json({ error: 'Failed to fetch dashboard stats' });
  }
});
